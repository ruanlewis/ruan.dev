...
export default function Strands({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const width = ctn.offsetWidth * scaleFactor;
      const height = ctn.offsetHeight * scaleFactor;
      renderer.setSize(width, height);
      
      // Ensure the canvas stretches perfectly even with lower internal resolution
    ...
    const update = (t: number) => {
      ...
      const isMobile = window.innerWidth < 768;
      if (isMobile) {
        // Cap mobile rendering to ~30 FPS to save phone battery and CPU
        const delta = t - lastTime;
        if (delta < 33.3) {
          return;
        }
        lastTime = t - (delta % 33.3);
      }
      ...
      // Track scroll position to modulate waves dynamically
      ...
      program.uniforms.uStrandCount.value = Math.min(
        Math.max(Math.round(isMobile ? Math.min(current.count, 2) : current.count), 1),
        MAX_STRANDS
      );
      
      // Modulate speed, amplitude and hue based on mouse only (calm, steady, and premium)
      program.uniforms.uSpeed.value = current.speed;
      program.uniforms.uAmplitude.value = current.amplitude * (1.0 + mouse.y * 0.25);
      ...
      program.uniforms.uHueShift.value = current.hueShift + mouse.x * 0.15;
